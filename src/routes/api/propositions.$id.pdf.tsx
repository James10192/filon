      ...
      POST: async ({ params, request }: { params: { id: string }; request: Request }) => {
        ...
        try {
          ...
          if (!preview.canFinalize) {
            return json({ message: 'Les éléments requis du document sont incomplets.' }, 422)
          }
          ...
          const reservationArgs: {
            ...
            documentType: 'devis' | 'proforma_hors_fne'
            contentHash: string
            snapshot: string
            language: 'fr' | 'en'
            ...
            documentType: previewModel.documentType,
            contentHash: documentHash(previewModel),
            snapshot: JSON.stringify(previewModel),
            language: previewModel.language,
            brandMode: previewModel.brandMode,
          }
          if (preview.organizationId) reservationArgs.organizationId = preview.organizationId

          const reservation = await fetchAuthMutation(
            api.billingProfiles.reserveProposalRevision,
            reservationArgs,
          )
          ...
          try {
            ...
            await page.setContent(
              renderProposalDocumentHtml(model, { baseUrl: new URL('/', request.url).toString() }),
          ...
          const finalized = await fetchAuthMutation(api.billingProfiles.finalizeProposalRevision, {
            revisionId,
            storageId: upload.storageId,
            filename: documentFilename(model),
            size: pdf.byteLength,
          })
          ...
          if (revisionId) {
            try {
              await fetchAuthMutation(api.billingProfiles.failProposalRevision, {
                revisionId,
                message: error instanceof Error ? error.message : 'pdf_generation_failed',
              })
            } catch {
              // L'erreur initiale reste la source de vérité pour le client.
            }
          }
