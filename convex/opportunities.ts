...
/**
 * `api.opportunities.unflagPriority` — retire le pointage. Autorisé au manager de
 * l'équipe OU au propriétaire (qui « solde » la priorité une fois traitée).
 */
export const unflagPriority = mutation({
  args: { id: v.id('opportunities') },
  handler: async (ctx, args) => {
    const { userId } = await requireUser(ctx)
...
/**
 * `api.opportunities.myFlagged` — vue focalisée du commercial : ses opportunités
 * pointées prioritaires par son head sell, plus récentes d'abord. Alimente le
 * bandeau « Priorités pointées » en tête de la page Opportunités.
 */
export const myFlagged = query({
  args: {},
  handler: async (ctx) => {
    const { userId } = await requireUser(ctx)
    const rows = await ctx.db
      ...
      .withIndex('by_user_flagged', (q) =>
        q.eq('userId', userId).eq('flaggedPriority', true),
      )
      .collect()
    const enriched = await enrichTargets(ctx, userId, rows)
    return enriched.sort((a, b) => (b.flaggedAt ?? 0) - (a.flaggedAt ?? 0))
  },
})
